...
def measurement_states(orthogonal_states: np.ndarray) -> np.ndarray:
    ...
    new_states[:, -1] = np.sqrt(-orthogonal_states[0]@orthogonal_states[1].conj())
...
def get_transformation_states(theta, d):
    ...
    Returns:
        The expanded set of states to transform into and the overlap matrix.
    """
    s = symmetric_states(theta, d)
    o = orthogonal_states(s)
    m = measurement_states(o)
    ...
    assert np.all(np.isclose((np.abs(cross_mat)**2).sum(axis=1),np.ones(d))), "Overlap isn't correct. Try a smaller angle"
